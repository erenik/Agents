...
public class ArchitectVolume extends Agent{
	...
	protected void setup() 
	{
        ...
        sd.setType("ArchitectVolume");
        sd.setName(getName());
        sd.setOwnership("The Architect itself");
        sd.addOntologies("ArchitectVolume");
        dfd.setName(getAID());
        dfd.addServices(sd);
        try {
...
class GUI extends JFrame {
	...
	public GUI() {
		...
		setTitle("Architect - Create soldier that spew volumes of data");
		...
		JLabel lblContent = new JLabel("Content (numBytes)");
		lblContent.setBounds(40, 120, 175, 14);
		contentPane.add(lblContent);
		
		JLabel lblPeriod = new JLabel("Periodicity (ms)");
		lblPeriod.setBounds(40, 164, 175, 14);
		contentPane.add(lblPeriod);
		
		ip = new JTextField();
		ip.setBounds(225, 40, 199, 20);
		ip.setText("tcp-368140612.us-west-2.elb.amazonaws.com");
		contentPane.add(ip);
		ip.setColumns(10);
